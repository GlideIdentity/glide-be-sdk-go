...
func ValidatePhoneNumber(phoneNumber string) error {
	...
	if !strings.HasPrefix(phoneNumber, "+") {
		return NewMagicalAuthError(ErrCodeInvalidPhoneNumber, "Phone number must be in E.164 format (start with +)")
	}

	if len(phoneNumber) < 8 {
		return NewMagicalAuthError(ErrCodeInvalidPhoneNumber, "Phone number too short for E.164 format (minimum 8 characters including +)")
	}

	if len(phoneNumber) > 16 {
		return NewMagicalAuthError(ErrCodeInvalidPhoneNumber, "Phone number too long for E.164 format (maximum 15 digits after +)")
	}
	...
	if !validFormat.MatchString(phoneNumber) {
		return NewMagicalAuthError(ErrCodeInvalidPhoneNumber, "Phone number contains invalid characters. E.164 format only allows + followed by digits")
	}

	// Detailed E.164 regex validation
	e164Regex := regexp.MustCompile(`^\+[1-9]\d{1,14}$`)
	if !e164Regex.MatchString(phoneNumber) {
		return NewMagicalAuthError(ErrCodeInvalidPhoneNumber, "Invalid E.164 phone number format")
	}
...
func ValidatePLMN(plmn *PLMN) error {
	...
	if !mccRegex.MatchString(plmn.MCC) {
		return NewMagicalAuthError(ErrCodeMissingRequiredField, "MCC must be exactly 3 digits")
	}

	// MNC validation (2 or 3 digits)
	mncRegex := regexp.MustCompile(`^\d{2,3}$`)
	if !mncRegex.MatchString(plmn.MNC) {
		return NewMagicalAuthError(ErrCodeMissingRequiredField, "MNC must be 2 or 3 digits")
	}
...
func ValidateUseCaseRequirements(useCase UseCase, phoneNumber string, plmn *PLMN) error {
	...
	case UseCaseGetPhoneNumber:
		...
		if phoneNumber != "" {
			return NewMagicalAuthError(ErrCodeInvalidUseCase, "Phone number should not be provided for GetPhoneNumber use case")
		}
		if plmn == nil {
			return NewMagicalAuthError(ErrCodeMissingRequiredField, "PLMN (MCC/MNC) is required for GetPhoneNumber use case")
		}

	case UseCaseVerifyPhoneNumber:
		// VerifyPhoneNumber: Need phone number, PLMN is optional
		if phoneNumber == "" {
			return NewMagicalAuthError(ErrCodeMissingRequiredField, "Phone number is required for VerifyPhoneNumber use case")
		}
	}
